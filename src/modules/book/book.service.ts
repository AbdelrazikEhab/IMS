...
export class BookService {
  constructor(
    ...
    @InjectQueue(QUEUE_NAMES.BOOK_PDF_WATERMARK)
  ...
  async create(tenantId: string, teacherId: string, dto: CreateBookDto) {
    const existing = await this.prisma.book.findFirst({
      where: { tenant_id: tenantId, slug: dto.slug },
    });
    if (existing) throw new ConflictException('Book slug already exists');
  ...
  async findById(tenantId: string, bookId: string) {
    ...
    // Never expose file_url directly
    const { file_url, ...safeBook } = book;
    return safeBook;
  }

  async update(tenantId: string, bookId: string, dto: UpdateBookDto) {
    const book = await this.prisma.book.findFirst({
      where: { id: bookId, tenant_id: tenantId },
    });
    if (!book) throw new NotFoundException('Book not found');
    ...
    if (dto.version !== undefined && book.version !== dto.version) {
      throw new ConflictException(
        'Book was modified. Please refresh and retry.',
      );
    }
  ...
  async delete(tenantId: string, bookId: string) {
    const book = await this.prisma.book.findFirst({
      where: { id: bookId, tenant_id: tenantId },
    });
    if (!book) throw new NotFoundException('Book not found');
  ...
  async publish(tenantId: string, bookId: string) {
    return this.prisma.book.update({
      where: { id: bookId },
  ...
  async unpublish(tenantId: string, bookId: string) {
    return this.prisma.book.update({
      where: { id: bookId },
  ...
  async getUploadUrl(tenantId: string, bookId: string) {
  ...
  async purchase(
    ...
    // In production: call payment gateway to get payment intent / checkout URL
    return {
  ...
  async getMyLibrary(tenantId: string, studentId: string) {
  ...
  async getDownloadUrl(tenantId: string, bookId: string, studentId: string) {
  ...
  async getOrders(tenantId: string, bookId: string) {
    return this.prisma.bookOrder.findMany({
